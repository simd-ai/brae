...
// term, a coefficient or an order.
static Foam::label braeDumpSSTIter()
{
    const char* e = ::getenv("BRAE_DUMP_STAGE_ITER");
    return e ? Foam::label(::atoi(e)) : Foam::label(-1);
}

// D() and the full right-hand side of an assembled system, in the form a brae-side capture can match:
// the diagonal INCLUDING the boundary internalCoeffs, and the source with boundaryCoeffs folded into
// their face cells. The off-diagonals go out beside them -- a per-cell view cannot see those, and they
// are what named the corrected-vs-orthogonal laplacian in the kEpsilon port.
static void braeDumpSSTSystem
(
    const Foam::fvMesh& mesh,
    const Foam::fvScalarMatrix& eqn,
    const Foam::word& dName,
    const Foam::word& sName
)
{
...
tmp<volScalarField> kOmegaSSTDumpBase<BasicEddyViscosityModel>::F1
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTDumpBase<BasicEddyViscosityModel>::F2() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTDumpBase<BasicEddyViscosityModel>::F3() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTDumpBase<BasicEddyViscosityModel>::F23() const
{
...
template<class BasicEddyViscosityModel>
void kOmegaSSTDumpBase<BasicEddyViscosityModel>::correctNut
(
...
template<class BasicEddyViscosityModel>
Foam::tmp<Foam::volScalarField> kOmegaSSTDumpBase<BasicEddyViscosityModel>::S2
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTDumpBase<BasicEddyViscosityModel>::Pk
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTDumpBase<BasicEddyViscosityModel>::epsilonByk
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTDumpBase<BasicEddyViscosityModel>::GbyNu0
(
    const volTensorField& gradU,
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTDumpBase<BasicEddyViscosityModel>::GbyNu
(
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTDumpBase<BasicEddyViscosityModel>::kSource() const
{
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTDumpBase<BasicEddyViscosityModel>::omegaSource() const
{
    return tmp<fvScalarMatrix>::New
    (
        omega_,
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTDumpBase<BasicEddyViscosityModel>::Qsas
(
    const volScalarField::Internal& S2,
...
template<class BasicEddyViscosityModel>
kOmegaSSTDumpBase<BasicEddyViscosityModel>::kOmegaSSTDumpBase
(
    const word& type,
...
template<class BasicEddyViscosityModel>
void kOmegaSSTDumpBase<BasicEddyViscosityModel>::setDecayControl
(
...
template<class BasicEddyViscosityModel>
bool kOmegaSSTDumpBase<BasicEddyViscosityModel>::read()
{
...
template<class BasicEddyViscosityModel>
void kOmegaSSTDumpBase<BasicEddyViscosityModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
    ...
    if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
    {
        ...
        volScalarField tmpl
        (
            IOobject("stage_sstDivU", mesh.time().timeName(), mesh,
                     IOobject::NO_READ, IOobject::NO_WRITE),
            mesh, dimensionedScalar(dimless, Zero), zeroGradientFvPatchScalarField::typeName
        );
        volScalarField dGb(tmpl);  dGb.rename("stage_sstGbyNu0");
        volScalarField dG(tmpl);   dG.rename("stage_sstG");
        volScalarField dS2(tmpl);  dS2.rename("stage_sstS2");
        tmpl.primitiveFieldRef() = divU;
        dGb.primitiveFieldRef()  = GbyNu0;
        dG.primitiveFieldRef()   = G;
        dS2.primitiveFieldRef()  = S2.primitiveField();
        tmpl.write(); dGb.write(); dG.write(); dS2.write();

        volScalarField("stage_sstCDkOmega", CDkOmega).write();
        volScalarField("stage_sstF1", F1).write();
        volScalarField("stage_sstF23", F23).write();
        volScalarField("stage_sstNutIn", nut).write();
        volScalarField("stage_sstKIn", k_).write();
        volScalarField("stage_sstOmegaIn", omega_).write();
        volVectorField("stage_sstU", U).write();
        surfaceScalarField("stage_sstAlphaRhoPhi", alphaRhoPhi).write();
        volTensorField
        (
            IOobject("stage_sstGradU", mesh.time().timeName(), mesh,
                     IOobject::NO_READ, IOobject::NO_WRITE),
            tgradU()
        ).write();
    }

    {
        ...
        if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
            braeDumpSSTSystem(this->mesh_, omegaEqn.ref(), "stage_sstOmD0", "stage_sstOmSrc0");

        omegaEqn.ref().relax();
        fvOptions.constrain(omegaEqn.ref());
        omegaEqn.ref().boundaryManipulate(omega_.boundaryFieldRef());
        if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
            braeDumpSSTSystem(this->mesh_, omegaEqn.ref(), "stage_sstOmD", "stage_sstOmSrc");
...

    {
        ...
        if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
            braeDumpSSTSystem(this->mesh_, kEqn.ref(), "stage_sstKD0", "stage_sstKSrc0");

        kEqn.ref().relax();
        fvOptions.constrain(kEqn.ref());
        if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
            braeDumpSSTSystem(this->mesh_, kEqn.ref(), "stage_sstKD", "stage_sstKSrc");
    ...
    if (this->mesh_.time().timeIndex() == braeDumpSSTIter())
    {
        volScalarField("stage_sstKOut", k_).write();
        volScalarField("stage_sstOmegaOut", omega_).write();
        volScalarField("stage_sstNutOut", this->nut_).write();
        if (this->mesh_.template foundObject<volScalarField>("alphat"))
            volScalarField
            (
                "stage_sstAlphatOut",
                this->mesh_.template lookupObject<volScalarField>("alphat")
            ).write();
    }
}
