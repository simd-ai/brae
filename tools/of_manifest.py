    ...
    "interFoam": [
        dict(name="interFoam_main", of_symbol="main",
             ...
             validation="A WHOLE TIME STEP'S ALPHA HALF NOW RUNS ON THE DEVICE "
                        ...
                        "numbers on two fields that had not moved. THE WHOLE STEP ON damBreak IS NOT "
                        "YET RIGHT: deviceInterStep runs clean on the synthetic fixture but produces "
                        "NaN in all 2268 cells within five steps of damBreak's developed state, and is "
                        "not gated there until that is found. That attempt also exposed a trap worth "
                        "carrying: std::fmax(a, NaN) returns a, so a worst-difference loop built on "
                        "fmax -- which is how nearly every arm in this tree accumulates -- reports "
                        "0.000e+00 for a field that has gone entirely non-finite. A finiteness check "
                        "has to precede any fmax accumulator. "
