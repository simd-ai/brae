    ...
    "interFoam": [
        ...
        dict(name="interFoam_kOmegaSST", of_symbol="kOmegaSSTBase",
             ...
             validation="tests/interfoam_waterchannel_vs_openfoam.sh, real OpenFOAM on RAS/waterChannel AS SHIPPED "
                        ...
                        "PROFILE oneCorrector, both arms: the atmosphere's U an inletOutlet in place of "
                        "pressureInletOutletVelocity and `nCorrectors 1`, twenty steps of 0.01 -- the one shape where "
                        "OpenFOAM's updated_ lag (the first corrector's evaluate keeps the assembly's valueFraction, "
                        "mixedFvPatchField.C:234-237) is the step's LAST evaluate, so the closure and the next assembly "
                        "read it; 708 of the 1400 atmosphere faces have flipped by the end. MEASURED: alpha 1.1e-12, "
                        "p_rgh 3.5e-12, U 5.3e-12, k 9.4e-14, omega 1.8e-12, nut 6.1e-13, all 20 counts. THREE "
                        "DEFECTS FOUND, each exact at step 1 and off at step 2, each on ONE arm: the host pushed the "
                        "new flux to U's patches after the corrector loop, so kOmegaSST's grad(U) read inletOutlet's "
                        "snGrad() with a switch OpenFOAM's patch had not moved yet (UEqn.A 3.2e-02 in the atmosphere "
                        "cells, U 2.7e-05, nut 2.4e-03; laminar twin 8.9e-12); the device's io switch at the "
                        "corrector's end read the new flux where the host lagged it (U 2.7e-05, nut 3.0e-01); and the "
                        "device's boundary gradient took an inletOutlet face's snGrad as (stored value - cell)*dc "
                        "where the class is MIXED and the switch is its valueFraction (laminar U 6.2e-09, HbyA.z "
                        "6.7e-07 in the atmosphere cells). CONTROL: OpenFOAM laminar against kOmegaSST, U 4.3e-03. "
                        "NOT DISCRIMINATED by any shipped profile: nCorrectors 2 re-evaluates every patch with the "
                        "moved switch before anything reads it.",
        ...
        dict(name="interFoam_fvOptions", of_symbol="fv::options",
             ...
             validation="tests/interfoam_angledduct_vs_openfoam.sh, real OpenFOAM on RAS/angledDuct (its own blockMesh, "
                        ...
                        "arms. THE DEVICE LOOP RUNS ALL THREE ARMS TOO (2026-09-20): inactive "
                        "alpha 6.1e-16, p_rgh 4.3e-15, U 7.6e-15; porous 1.1e-14, 1.3e-11, 1.8e-11; porousWater "
                        "5.9e-13, 2.9e-12, 2.6e-12, with all 30 of the device's OWN p_rgh iteration counts "
                        "OpenFOAM's in each. The device carries the FULL transformed D and F tensors, which the "
                        "other solvers' device porosity does not -- they take the diagonal and refuse a rotated "
                        "coordinateSystem by name, and this tutorial rotates e1 by 45 degrees -- and mu and rho as "
                        "FIELDS (DevicePorosity::tensorForm, kernels transcribed from fvOptions_cpp.cu:502-537). "
                        "BROKEN ONCE EACH on the device: the porosity dropped p_rgh 7.9e-01, U 2.3e-01; handed the "
                        "kinematic nu rather than rho*nu the same, since the case's F is zero. AND THE ARM FOUND "
                        "THREE MORE DEFECTS, none of them the option's, each localised with the HOST closure run "
                        "inside the device loop: the massFlowRate inlet never rebuilt on the device (U 1.0007e+00); "
                        "the SLIP wall's symmetry refValue never refreshed against this iteration's cells, which is "
                        "the third step of rhoSimpleFoam's own documented pre-assembly sequence (rhoUEqn.cuh:76-78) "
                        "and was simply missing from interFoam's step (U 2.7e-02 with the host closure and the "
                        "option off, where the host loop is 4.1e-15); and the device closure's turbulent inlets "
                        "frozen at the file's `value` because interFoam never handed it their masks (U 1.3e+00, k "
                        "1.1e+00, epsilon 4.7e+00), plus the wall-function production reading a WALL VELOCITY "
                        "snapshotted at the closure's build, exact for every noSlip wall and wrong for a slip one "
                        "(epsilon 1.5e-02, worst on `porosityWall`). NOT CLAIMED, each refused by name: every other "
                        "option type, explicitPorositySource's fixedCoeff model, more than one active porosity zone "
                        "on the device, an option under a moving mesh or beside an MRF zone; the Forchheimer half "
                        "(f is zero in the tutorial).",
        ...
        dict(name="interFoam_variableHeightFlowRate", of_symbol="variableHeightFlowRateInletVelocityFvPatchVectorField",
             ...
             validation="tests/interfoam_weiroverflow_vs_openfoam.sh, real OpenFOAM on RAS/weirOverflow AS SHIPPED (its "
                        ...
                        "OpenFOAM's text, with an inletOutlet as the control. THE DEVICE LOOP RUNS THIS CASE "
                        "NOW (2026-09-20), at the same bounds: alpha 2.4e-13, p_rgh 4.3e-13, U 5.1e-11, its inlet "
                        "5.6e-13 face by face. It took ddtCorr's BOUNDARY half in the device pressure step (the "
                        "device already computed it, already zeroed where U fixes a value, and dropped it: BROKEN "
                        "U 3.6e-05, p_rgh 1.3e-06 against a bound of 5.0e-10) and the velocity condition rebuilt in "
                        "the U-boundary hook from the phase fraction on its patch (BROKEN U 1.0000e+00, the inlet "
                        "3.6e+00). alpha's own condition also drove a change -- MULES's per-face fixesValue mask is "
                        "now rebuilt from the patch's valueFraction rather than taken once from the patch's "
                        "fixesValue() -- and THAT IS NOT CLAIMED: the mask varies here (13 of 44 faces outflow at "
                        "the end, asserted by the gate) but the answer does not depend on it, since dropping the "
                        "refresh changes no digit and forcing the mask to zero over the whole patch moves p_rgh from "
                        "4.2981e-13 to 4.3078e-13 and nothing else. NOT CLAIMED: a flowRate that varies in time "
                        "(refused). AND THE ARM FOUND A DEFECT IN ANOTHER CASE: adding the boundary half put "
                        "RAS/angledDuct's `inactive` arm 1.4e-11 from OpenFOAM where it had been 7.6e-15, because the "
                        "device formed it from the face CELL's U.oldTime() while fvc::dotInterpolate(Sf, U.oldTime()) "
                        "on an uncoupled patch is `pSf & vf.boundaryField()[pi]`, the STORED patch value "
                        "(surfaceInterpolationScheme.C:296-298) -- the same number wherever the patch value follows "
                        "the cell, and not on a SLIP wall. U's patch values are snapshotted with its cells now. "
                        "STILL RED and not from this work: the k FINAL-residual arm, OpenFOAM 8.189e-14 "
                        "against brae 8.092e-14, two stopping points at round-off, awaiting a decision.",
