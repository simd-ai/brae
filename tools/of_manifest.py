    ...
    "interFoam": [
        dict(name="interFoam_main", of_symbol="main",
             ...
             validation="A WHOLE TIME STEP'S ALPHA HALF NOW RUNS ON THE DEVICE "
                        ...
                        "8.5e-05 host against 5.0e-05 device. It REFUSES adjustTimeStep by name -- "
                        "deviceAlphaCourantNo is ported but nothing feeds it back into deltaT yet, and "
                        "running a fixed step where the case asks for an adaptive one would silently "
                        "solve a different problem. (The report's maxU and worstDivPhi had to be filled "
                        "too: left at their defaults the device path printed `max|U| 0 m/s, worst "
                        "|div(phi)| 0.000e+00` on a run whose U reaches 0.27.) "
