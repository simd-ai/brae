    ...
    "interFoam": [
        ...
        dict(name="interFoam_waveModel", of_symbol="waveModel",
             ...
             validation="tests/interfoam_waves_vs_openfoam.sh, real OpenFOAM run SERIALLY, fixed steps, on ALL NINE "
                        ...
                        "absorption left out 2.1e-01. ONE STAGING CHANGE IN EVERY PROFILE THAT IS NOT ABOUT WAVES: the "
                        "tutorial solves p_rghFinal with GAMG, which brae's interFoam substitutes under a notice, so the "
                        "staging names PCG with DIC. tests/interfoam_refusals.sh holds fifteen wave and named-flux arms. "
                        "WHAT IT DOES NOT CLAIM: nPaddle above 1 and a non-zero waveAngle (every tutorial names one "
                        "paddle and angle 0, the 3-D ones included; only irregularMultiDirectional's per-component "
                        "directions put a wave at an angle), a patch that is not axis-aligned (the local frame's tensor "
                        "is diagonal here, so OpenFOAM's pairing of Rgl and Rlg cannot be told from its transpose), a "
                        "restart, the device loop, and two timings no fixture could see -- below.",
             note="waveAlpha AND waveVelocity ARE fixedValue PATCHES WHOSE VALUE A MODEL SUPPLIES, one model per "
                  ...
                  "rhoPhi accordingly and refuses any other name; `-device` and the turbulence closure, which hand "
                  "over phi only, refuse a patch that names anything else. Other solvers still ignore the entry -- "
                  "they carry one flux -- and that is recorded here rather than claimed. TWO TIMINGS ARE "
                  ...
                  "with the wave, so it gated noise. ON THE HOST, as every other boundary condition of this solver "
                  "is: the device loop reaches them through per-patch hooks. It has no point in its alpha step to "
                  "call this one from yet, so `-device` on a wave case is refused by name rather than run with the "
                  "inlet frozen."),
