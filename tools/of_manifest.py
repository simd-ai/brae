    ...
    "interFoam": [
        ...
        dict(name="interFoam_alphaEqn", of_symbol="alphaEqn",
             ...
             note="SPLIT IN TWO. The flux assembly is landed: alphaControls, the off-centring, phic, "
                  ...
                  "8.2e-02. ONE FIX HERE IS FROM THE SOURCE AND NOT FROM A MEASUREMENT: the host passed "
                  "phiCN, the VOLUMETRIC flux, as MULES::correct's flux argument where OpenFOAM passes "
                  "alphaPhi10 (alphaEqn.H:140). limiterCorr reads it in one place, the boundary outlet test "
                  "`(phi_b + phiCorr_b) > SMALL*SMALL` (CMULESTemplates.C:546) -- the same slip the "
                  "corrector loop had, fixed there at 3.99e-09 -> 1.07e-10. Here the host's numbers are "
                  "unchanged to five digits, because the boundary half of the cached correction is zero "
                  "unless alpha's patch value moves between the pre-solve and the correctors, which needs "
                  "water leaving through an outflow face; no fixture here does that."),
