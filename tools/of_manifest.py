    ...
    "interFoam": [
        ...
        dict(name="interFoam_turbulence", of_symbol="incompressibleInterPhaseTransportModel",
             ...
             validation="tests/interfoam_ras_dambreak_vs_openfoam.sh, real OpenFOAM on RAS/damBreak, five fixed steps of "
                        ...
                        "16 turbulence arms on RAS/damBreak itself. WHAT IT DOES NOT CLAIM: any nut wall function but "
                        "nutk (nutU and nutLowRe are accepted because the shared closure gates them under rhoSimpleFoam, "
                        "not here); kMin/epsilonMin, which bound() never reaches on this case; a non-orthogonal mesh; "
                        "more than one outer corrector with the closure on.",
             note="incompressibleInterPhaseTransportModel IS TWO MODELS BEHIND ONE KEYWORD, and brae's own refusal "
                  ...
                  "equation's own mass flux and in this lineage is NOT (the equation convects with rhoPhi). The "
                  "sign argument says it cannot matter; the measurement says 4.9e-04, because rhoPhi's patch values "
                  "are not rho_b*phi_b at the first step. (3) A DEFECT IN THE SHARED CLOSURE, found by the `custom` "
                  "profile: epsilon agreed with OpenFOAM to 1e-13 while every initial and final residual of its "
                  "solve sat 1.1e-04 away, only under relaxation, only for epsilon. OpenFOAM's epsilonWallFunction "
                  "is a fixedValue patch, so at relax() its faces carry the laplacian's coefficient ic and the "
                  "diagonal becomes max(|D0 + ic|, sumOff)/alpha - ic; brae maps the patch to zeroGradient and got "
                  "D0/alpha. setValues pins exactly those rows next, so the solution cannot see it, but the "
                  "diagonal survives into normFactor and a residual is what the stopping rule reads. Fixed in the "
                  "host reference: 1.1e-04 -> 4.6e-14. STILL OPEN, and not measured: the device twin kEpsilon.cu "
                  "does not carry the term, and neither kOmegaSST closure does -- omegaWallFunction has the same "
                  "fixedValue base class (omegaWallFunctionFvPatchScalarField.H:88-90), so the same term is "
                  "expected there, which is a lead and not a measurement. It moves a residual by 1e-04, so it can "
                  "matter only where a solve stops within 1e-04 of its tolerance; rhoSimpleFoam's gates, which "
                  "relax at 0.7, pass with and without it. TWO LATENT SUBSTITUTIONS THE TURBULENCE REFUSAL HAD "
                  "BEEN HIDING, found by asking what else would run once it lifted. `ddtSchemes default` was parsed "
                  "into f.ddtU and handed to nobody, so CrankNicolson (floatingObject) and localEuler (DTCHull) "
                  "would have run as Euler: now refused in buildInterFields. And laplacianSchemes/snGradSchemes "
                  "were never opened: brae assembles the pressure laplacian and its three snGrads orthogonal while "
                  "28 of 44 tutorials say `corrected` or `limited`. On a mesh of rectangles that is not a "
                  "substitution (the correction vector is zero; it is why damBreak agrees to 1e-12 under `Gauss "
                  "linear corrected`); on any other it is, so a corrected scheme on a mesh more than 1e-10 off "
                  "orthogonal is refused, and the refusals gate shears damBreak six degrees to prove both sides. "
                  "`minIter` is honoured for k and epsilon and REFUSED for the alpha pre-solve and the momentum "
                  "predictor, which drop it (three tutorials name it for alpha). THE CENSUS CAUGHT ITS OWN DEFECT: "
                  "run with blockMesh alone it showed damBreakLeakage and damBreakPorousBaffle reaching End, on "
                  "meshes without the baffles their Allrun creates. Prepared by their own Allrun utilities both are "
                  "refused (porousBafflePressure by name; the leakage case in the mesh reader, on a cyclicACMI "
                  "`coded` block -- a refusal, but not a named one). 33 tutorials prepare cleanly: 3 reach End and "
                  "all 3 are gated; waves 9, moving meshes 12. GPU_REQUIRED AND NOT YET ON THE GPU: the target "
                  "is the closure's existing device twin, which the interFoam device loop does not call. "
                  "Until it does, `-device` on a RAS case is refused by name rather than run laminar."),
