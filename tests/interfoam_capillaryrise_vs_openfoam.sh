#!/usr/bin/env bash
# brae's interFoam against REAL OpenFOAM's interFoam, on capillaryRise, field by field.
#
# WHY THIS CASE AND NOT ONLY damBreak. On damBreak surface tension is a rounding correction -- gravity
# and inertia decide everything, and a solver with sigma set to zero would still look broadly right for
# a while. On capillaryRise the surface tension IS the answer: the water climbs the tube because of it,
# against gravity, at an angle the WALL sets. It is also the only shipped interFoam tutorial with a
# contact angle (constantAlphaContactAngle, theta0 45, limit gradient), so it is the only case in the
# whole tutorial set that exercises correctContactAngle at all.
#
...
BIN="${BUILD:-$ROOT/build}/test_inter_capillary_vs_openfoam"
