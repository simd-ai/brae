#!/usr/bin/env bash
# brae's interFoam WAVE boundary conditions -- waveAlpha, waveVelocity, and the StokesI and
# shallowWaterAbsorption models behind them -- against REAL OpenFOAM on laminar/waves/stokesI.
#
...
#
# THE CONTROL is OpenFOAM's own answer for the same tank with waveHeight 0: the wave moves its U by
# 100% and its alpha by 0.73, against brae's 1.8e-08 and 1.4e-10 from it.
#
...
# stage <name> <deltaT> <nSteps> <nx|-> <waves: shipped|trough|crest|still> <solves: case|tight|mompred>
stage()
{
    local name="$1" dt="$2" n="$3" nx="$4" waves="$5" solves="$6"
    local C="$W/$name"
    cp -r "$SRC" "$C" || return 1
...
if nx != '-':
    ...
    t = open(b).read()
    t, k = re.subn(r'\(\s*\d+\s+1\s+(\d+)\s*\)\s*simpleGrading', r'(%s 1 \1) simpleGrading' % nx, t)
...
assert k1 == 1 and k2 == 1, 'p_rghFinal is no longer GAMG with a DIC smoother'
...
if waves == 'still':
    t, k = re.subn(r'waveHeight\s+[^;]+;', 'waveHeight      0;', t)
    assert k == 1, 'waveHeight not found'
    t = re.sub(r'rampTime\s+[^;]+;', 'rampTime        0.05;', t)
open(w, 'w').write(t)
PYEOF
...
rc=0
stage shippedStill 0.01 10 -   still   case  || rc=1
stage shipped      0.01 10 -   shipped case  || rc=1
stage smallStill   0.01 20 100 still   case  || rc=1
stage trough       0.01 20 100 trough  case  || rc=1
stage crest        0.01 20 100 crest   case  || rc=1
stage tight        0.01 20 100 trough  tight || rc=1
stage mompred      0.01 20 100 trough  mompred || rc=1
