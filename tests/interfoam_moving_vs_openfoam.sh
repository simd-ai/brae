...
if profile.startswith('mixer'):
    # the two staging changes the header declares
    m = re.search(r'p_rghFinal\s*\{.*?\n    \}', t, flags=re.S)
    assert m and 'GAMG' in m.group(0) and 'preconditioner' in m.group(0), 'p_rghFinal is no longer PCG with a GAMG preconditioner'
