    ...
    const DeviceBuffer<scalar>* nutwStored)   // see the header
{
    ...
    if (w.nWC > 0)
    {
        ...
        const scalar kappa = co.kappa, E = co.E;
        const bool lowReCorrection = co.epsLowRe;
        scalar *eps0d = eps0.data(), *G0d = G0.data();
        ...
        const scalar* wfCmu25d  = w.wfCmu25.size()  ? w.wfCmu25.data()  : nullptr;
        const scalar* wfCmu75d  = w.wfCmu75.size()  ? w.wfCmu75.data()  : nullptr;
        const scalar* wfKappad  = w.wfKappa.size()  ? w.wfKappa.data()  : nullptr;
        const scalar* wfEd      = w.wfE.size()      ? w.wfE.data()      : nullptr;
        const scalar* wfYplLamd = w.wfYplLam.size() ? w.wfYplLam.data() : nullptr;
        pcudaParallelFor(nBlocks(nWC), TPB, [=] __device__ () {
            wallFnKernel(nWC, wcCell, wcStart, wcFace, wfY, wfDc, wux, wuy, wuz, invNw, kd, Uxd, Uyd, Uzd,
                        nu, yplLam, Cmu25, Cmu75, kappa, E, atmZ0, atmBoundNut, nutWall, lowReCorrection,
                        eps0d, G0d, nuFaced, nutwStoredd, wfCmu25d, wfCmu75d, wfKappad, wfEd, wfYplLamd);
        });
    }
