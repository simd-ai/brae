...
#include <climits>   // INT_MAX etc.: nvcc pulls this in transitively, PCUDA's minimal runtime does not

#ifndef __forceinline__
#define __forceinline__ inline   // PCUDA has no forced-inline attribute; a hint only, so this is a no-op
#endif

// __launch_bounds__(...) doesn't parse when it lands between PCUDA's __global__ macro expansion and the
// return type (see doc/pcuda.md; __global__ becomes a block of __attribute__ annotations). Register-count
// hint only -- dropping it costs nothing but a missed occupancy hint.
