    ...
    const DeviceBuffer<scalar>* nutwStored)   // the STORED wall nut, wall-face order; null -> recompute
{
    ...
    {
        const int nWC = w.nWC;
        const label *wcCell = w.wcCell.data(), *wcStart = w.wcStart.data(), *wcFace = w.wcFace.data();
        ...
        const scalar* nutwStoredd = (nutwStored && nutwStored->size()) ? nutwStored->data() : nullptr;
        pcudaParallelFor(nBlocks(nWC), TPB, [=] __device__ () {
            wallOmegaG0Kernel(nWC, wcCell, wcStart, wcFace, wfY, wfDc, wux, wuy, wuz, invNw, kd, Uxd, Uyd, Uzd,
                              nu, yplLam, Cmu25, kappa, E, atmZ0, atmBoundNut, beta1, nutWall, omega0d, G0d,
                              nuFaced, nutwStoredd);
        });
    }
    ...
    DeviceBuffer<scalar>& Fth, DeviceBuffer<scalar>& spR, DeviceBuffer<scalar>& suR)
{
    ...
    {
        const scalar *gradUd=gradU.data(), *Uxd=Ux.data(), *Uyd=Uy.data(), *Uzd=Uz.data();
        const scalar *kd=k.data(), *omegad=omega.data(), *yd=y.data(), *ReThetatd=ReThetat.data(), *gammaIntd=gammaInt.data();
        const scalar cThetat=lm.cThetat, ce2=lm.ce2, lambdaErr=lm.lambdaErr;
        scalar *Fthd=Fth.data(), *spRd=spR.data(), *suRd=suR.data(); label* worstd = worst.data();
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
            lmReThetatPrepKernel(nC, gradUd, Uxd, Uyd, Uzd, kd, omegad, yd, ReThetatd, gammaIntd, nu,
                                 cThetat, ce2, LM_SMALL, lambdaErr, Fthd, spRd, suRd, worstd);
        });
    }
    ...
    bool gsSymmetric)
{
    ...
    DeviceBuffer<scalar> DRe(nC);
    {
        ...
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () { lmReDiffKernel(nC, nutd, sigmaThetat, nu, DRed); });   // sigmaThetat*(nut+nu)
    }
    ...
    DeviceBuffer<scalar> spG(nC), suG(nC);
    {
        ...
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
            lmGammaPrepKernel(nC, gradUd, Uxd, Uyd, Uzd, kd, omegad, yd, ReThetatd, gammaIntd, nu, ca1, ca2, ce1, ce2, LM_SMALL, spGd, suGd);
        });
    }
    ...
    gammaIntEff.resize(nC);
    {
        ...
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
            lmGammaEffKernel(nC, gradUd, Uxd, Uyd, Uzd, kd, omegad, yd, ReThetatd, gammaIntd, Fthd, nu, LM_SMALL, gammaIntEffd);
        });
    }
