...
void deviceDiv(const DeviceMesh& dm, const DeviceBuffer<scalar>& phiInt, const DeviceBuffer<scalar>& bval, DeviceBuffer<scalar>& d)
{
    ...
    const label* bndIsEmptyD = dm.bndIsEmpty.data();
    const scalar* bvalD = bval.data(); const scalar* Vd = dm.V.data(); scalar* dd = d.data();
    pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
        divKernel(nC, ownerStart, losort, losortStart, phiIntD, bndCellStart, bndPerm, bndIsEmptyD, bvalD, Vd, dd); });
    ...
    const int* skipIf)
{
    ...
    gz.resize(dm.nCells);
    {
        ...
        const label* bndCellStart = dm.bndCellStart.data(); const label* bndPerm = dm.bndPerm.data();
        const label* bndGFace = dm.bndGFace.data(); const label* bndIsEmpty = dm.bndIsEmpty.data();
        const scalar* bvalD = bval.data();
        const scalar* Vd = dm.V.data(); scalar* gxd = gx.data(); scalar* gyd = gy.data(); scalar* gzd = gz.data();
        const int* skipIfD = skipIf;
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
            gradKernel(nC, own, nei, w, Sfx, Sfy, Sfz, vold, ownerStart, losort, losortStart,
                       bndCellStart, bndPerm, bndGFace, bndIsEmpty, bvalD, Vd, gxd, gyd, gzd, skipIfD); });
    }
    ...
    int nIfs)
{
    ...
    if (nIfFaces > 0)
    {
        ...
        }
        {
            ...
            pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
                cellLimitFactorKernel(nC, ownerStart, losort, losortStart, bndCellStart, bndPerm, bndIsEmpty,
    ...
    }
    {
        ...
        pcudaParallelFor(nBlocks(nC), TPB, [=] __device__ () {
            cellLimitGradKernel(nC, k, Ud, Ubndd, ownerStart, nei, losort, losortStart, owner,
                                 bndCellStart, bndPerm, bndIsEmpty, dOwnX, dOwnY, dOwnZ, dNeiX, dNeiY, dNeiZ,
