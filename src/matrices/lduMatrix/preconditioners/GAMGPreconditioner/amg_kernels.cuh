...
template <typename T>
inline void restrictGatherTLaunch(int nCoarse, const label* cellStart, const label* cellList, const T* r, T* rc)
{
    pcudaParallelFor(nBlocks(nCoarse), TPB, [=] __device__ () {
        restrictGatherT<T>(nCoarse, cellStart, cellList, r, rc); });
}
template <typename T>
__device__
void prolongT(
